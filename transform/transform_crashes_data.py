...
cleaned_data = cleaned_data.withColumn("year", year(col("crash_date")))\
...
for vehicle_col in ["vehicle_type_code1", "vehicle_type_code2"]:
    cleaned_data = cleaned_data.withColumn(vehicle_col + "_involved",  when(col(vehicle_col).isNotNull(), 1).otherwise(0))

cleaned_data = cleaned_data.withColumn("total_vehicles_involved", col("vehicle_type_code1_involved") + col("vehicle_type_code2_involved"))
