...
def setup_parser():
    ...
    parser.add_argument("--job-name", dest="job_name", help="Name of the Dataflow job.", default="taxi-data-landing-to-raw")
    ...
    parser.add_argument("--input-pattern", dest="input_pattern", help="GCS path pattern for input files.", default="gs://df-landing-zone/data/pre-processed/taxi_data/*.json")
    return parser
...
def typecast_data(element):
    ...
    cast_functions = {
        'VendorID': int,
        'tpep_pickup_datetime': lambda x: datetime.datetime.strptime(x, '%Y-%m-%d %H:%M:%S') if isinstance(x, str) else datetime.datetime.strftime(x, '%Y-%m-%d %H:%M:%S'),
        'tpep_dropoff_datetime': lambda x: datetime.datetime.strptime(x, '%Y-%m-%d %H:%M:%S') if isinstance(x, str) else datetime.datetime.strftime(x, '%Y-%m-%d %H:%M:%S'),
        'passenger_count': int,
        'trip_distance': float,
        'RatecodeID': int,
